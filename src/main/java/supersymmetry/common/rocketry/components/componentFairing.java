...
public class componentFairing extends AbstractComponent<componentFairing> {
  ...
  //   //     .checkHull(
  //   //         aabb,
  //   //         analysis.getBlockConn(aabb, analysis.getBlocks(analysis.world, aabb,
  // true).get(0)),
  //   //         false)
  //   //     .getSecond()
  //   //     .isEmpty()) {
  ...
  // blocks.stream().filter(connCheck).collect(Collectors.toSet());
  //   // These connector blocks should form a ring, with their primary directions not facing any
  // other
  //   // block
  ...
  //   // If there are more than 2 partitions of air in the bounding box, then there's an
  // abnormality
  //   // with the hull shape.
  ...
  //   }
  //
  //   // Objective: We don't really want tiles on the inside of the fairing - that wouldn't make
  // any
  //   // sense!
  //   // So, we're going to obtain the fairing with a ceiling (if it didn't have one, it wouldn't
  // have
  //   // )
  //   // Note that
  ...
  public Optional<NBTTagCompound> analyzePattern(
    ...
    Predicate<BlockPos> connCheck =
        bp -> world.getBlockState(bp).getBlock().equals(SuSyBlocks.FAIRING_CONNECTOR);
    Set<BlockPos> connectorBlocks =
    ...
    for (BlockPos blockFace : connectorBlocks) {
      ...
      if (analysis.getCoordOfAxis(blockFace) != axisPos || dir_general != dir) {
        ...
        if (intPartition.contains(pointingTo)
            || extPartition.contains(pointingTo)
    ...
    while (!collectedConnectors.containsAll(connectorBlocks)) {
      ...
      for (BlockPos initiate : toConnect) {
        Set<BlockPos> blockNeighbors =
            analysis.getBlockNeighbors(initiate, fairingBB, StructAnalysis.neighborVecs).stream()
    ...
    for (BlockPos bp : blocksConnected) {
      ...
      for (EnumFacing facing : EnumFacing.VALUES) {
        ...
        if (world.getBlockState(bp).getValue(FACING).equals(facing)
            && b instanceof BlockFairingConnector) {
          expectation = false;
        }
        if (expectation
            ^ te.isCovered(
    ...
    Double bottomSemiRadius =
        analysis.getApproximateRadius(analysis.getLowestLayer(blocksConnected));
