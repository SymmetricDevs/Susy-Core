...
public class MaterialCost {
    ...
    public List<GTRecipeInput> expandRecipe(RecipeMap<?> map, long maxEnergy) {
        ...
        if (itemCache.containsKey(selfStack)) {
            ...
            List<List<GTRecipeInput>> possibleInputs = possibleRecipes.stream()
                    .map(x -> x.getInputs())
                    .distinct()
            ...
            selected = possibleRecipes.stream()
        ...
        ItemStack outputStack = selected.getOutputs().stream()
                .filter(
                        x -> {
                            return x.getItem() == selfStack.getItem() && x.getMetadata() == selfStack.getMetadata();
                        })
                .findFirst()
