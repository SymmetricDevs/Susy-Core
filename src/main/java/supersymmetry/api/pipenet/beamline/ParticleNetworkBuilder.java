 ...
 */
public class ParticleNetworkBuilder extends Thread {

    private final ObjectList<BlockPos> starts = new ObjectArrayList<>();
    private final ParticleNetwork.WorldData worldData;
    private final ParticleNetwork originalNetwork;
    private ParticleNetwork network;
    ...
    public void run() {
        ...
        while (!this.starts.isEmpty()) {
            ...
            ParticleNetwork ldn = this.worldData.getNetwork(start);
            ...
            this.network = this.network.getBeamLineType().createNetwork(this.worldData);
    ...
    private void checkPos(IBlockState blockState, BlockPos pos) {
        ParticleNetwork network = ParticleNetwork.get(this.world, pos);
        if (network != null && network != this.network) {
            network.invalidateNetwork(true);
        }
        BlockPos bp = pos.toImmutable();
        this.walked.add(bp);
        IParticleNetworkPart part = IParticleNetworkPart.tryGet(this.world, pos, blockState);
        if (part != null) {
            this.pipes.add(bp);
            if (part instanceof IBeamLineEndpoint endpoint) this.endpoints.add(endpoint);
