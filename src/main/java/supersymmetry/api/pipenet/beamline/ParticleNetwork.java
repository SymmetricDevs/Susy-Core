...
public class ParticleNetwork {
    ...
    private final ObjectOpenHashSet<BlockPos> beamLineBlocks = new ObjectOpenHashSet<>();
    ...
    protected ParticleNetwork(BeamLineType beamLineType, WorldData world) {
        this.beamLineType = beamLineType;
        this.world = world;
    }

    @Nullable
    public static ParticleNetwork get(World world, BlockPos pos) {
    ...
    protected void recalculateNetwork(Collection<BlockPos> starts) {
        ...
        new ParticleNetworkBuilder(world, this, starts).start();
    }

    /**
     * Called from the {@link ParticleNetworkBuilder} to set the gathered data
     */
    protected void setData(Collection<BlockPos> pipes, List<IBeamLineEndpoint> endpoints) {
        invalidateEndpoints();
        this.beamLineBlocks.clear();
        this.beamLineBlocks.addAll(pipes);
        this.endpoints.clear();
        this.endpoints.addAll(endpoints);
        if (this.beamLineBlocks.isEmpty()) {
            invalidateNetwork(false);
            return;
        }
        for (BlockPos pos : this.beamLineBlocks) {
    ...
    public void onRemovePipe(BlockPos pos) {
        ...
        this.beamLineBlocks.remove(pos);
        ...
        for (EnumFacing facing : EnumFacing.VALUES) {
            ...
            ParticleNetwork network = world.getNetwork(offsetPos);
            if (network == this) {
    ...
    protected void addEndpoint(IBeamLineEndpoint endpoint) {
        if (!this.endpoints.contains(endpoint)) {
            this.endpoints.add(endpoint);
        }
    }

    protected void addEndpoint(Collection<IBeamLineEndpoint> endpoints) {
        for (IBeamLineEndpoint endpoint : endpoints) {
            if (!this.endpoints.contains(endpoint)) {
                this.endpoints.add(endpoint);
            }
        }
    ...
    public void onRemoveEndpoint(IBeamLineEndpoint endpoint) {
     ...
     */
    public void onPlaceEndpoint(IBeamLineEndpoint endpoint) {
        addEndpoint(endpoint);
        this.beamLineBlocks.add(endpoint.pos());
     ...
     */
    protected void mergePipeNet(ParticleNetwork network) {
        if (getBeamLineType() != network.getBeamLineType()) {
            throw new IllegalStateException("Can't merge unequal pipe types, " + getBeamLineType().getName() + " and " +
                    network.getBeamLineType().getName() + " !");
        }
        for (BlockPos pos : network.beamLineBlocks) {
            this.world.putNetwork(pos, this);
            this.beamLineBlocks.add(pos);
        }
        addEndpoint(network.endpoints);
        for (IBeamLineEndpoint endpoint1 : this.endpoints) {
    ...
    public void invalidateEndpoints() {
        ...
        for (IBeamLineEndpoint endpoint : this.endpoints) {
            endpoint.invalidateLink();
        }
    }
     ...
     */
    @Nullable
    public IBeamLineEndpoint getOtherEndpoint(IBeamLineEndpoint endpoint) {
        ...
        if (isIOIndexInvalid()) {
            ...
            IBeamLineEndpoint in = this.endpoints.get(this.activeInputIndex);
            IBeamLineEndpoint out = this.endpoints.get(this.activeOutputIndex);
            if (!this.beamLineType.satisfiesMinLength(in, out)) {
                invalidateEndpoints();
        ...
        if (otherIndex >= 0) {
            ...
            IBeamLineEndpoint other = this.endpoints.get(otherIndex);
    ...
    private int find(IBeamLineEndpoint endpoint) {
        for (int i = 0; i < this.endpoints.size(); i++) {
            IBeamLineEndpoint other = this.endpoints.get(i);
            if (!other.isValid()) {
            ...
            if (endpoint != other &&
                    ...
                    this.beamLineType.satisfiesMinLength(endpoint, other)) {
    ...
    public IBeamLineEndpoint getActiveInputIndex() {
        return this.activeInputIndex >= 0 ? this.endpoints.get(this.activeInputIndex) : null;
    }

    public IBeamLineEndpoint getActiveOutputIndex() {
    ...
    public int getTotalSize() {
        return this.beamLineBlocks.size();
    }
    ...
    public BeamLineType getBeamLineType() {
        return beamLineType;
    }
    ...
    public static class WorldData extends WorldSavedData {
        ...
        private final Long2ObjectMap<Object2ObjectMap<BlockPos, ParticleNetwork>> networks = new Long2ObjectOpenHashMap<>();
        // All existing networks in this world
        private final ObjectOpenHashSet<ParticleNetwork> networkList = new ObjectOpenHashSet<>();
        ...
        protected void setWorldAndInit(World world) {
            if (this.worldRef.get() != world) {
                for (ParticleNetwork ld : this.networkList) {
                    if (!ld.endpointPoss.isEmpty()) {
                        ld.endpoints.clear();
                        for (BlockPos pos : ld.endpointPoss) {
                            IBeamLineEndpoint endpoint = IBeamLineEndpoint.tryGet(world, pos);
                            if (endpoint != null) {
        ...
        public ParticleNetwork getNetwork(BlockPos pos) {
            return this.networks.getOrDefault(getChunkPos(pos), Object2ObjectMaps.emptyMap()).get(pos);
        }

        private void putNetwork(BlockPos pos, ParticleNetwork network) {
            long chunkPos = getChunkPos(pos);
            Object2ObjectMap<BlockPos, ParticleNetwork> chunkNetworks = this.networks.get(chunkPos);
        ...
        public void readFromNBT(@NotNull NBTTagCompound nbtTagCompound) {
            ...
            for (NBTBase nbt : list) {
                ...
                BeamLineType pipeType = BeamLineType.getBeamLineType(tag.getString("class"));
                ParticleNetwork ld = pipeType.createNetwork(this);
        ...
        public NBTTagCompound writeToNBT(@NotNull NBTTagCompound nbtTagCompound) {
            ...
            for (ParticleNetwork network : this.networkList) {
                NBTTagCompound tag = new NBTTagCompound();
                list.appendTag(tag);

                String name = network.getBeamLineType().getName();
                ...
                for (IBeamLineEndpoint endpoint : network.endpoints) {
