...
public class MissingMods {
    ...
    public static final String[][] REQUIRED_MODS = { { "srparasites", "Scape and Run Parasites" },
            { "openglasses", "OpenGlasses" }, { "rendertoolkit", "Commons0815 (renderToolkit)" }, };

    /**
     * Fires on the PHYSICAL CLIENT when the player connects to any server. Warns
     * the player if their client is missing any required mods.
     */
    ...
    public static void onClientJoinServer(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        ...
        net.minecraft.client.Minecraft.getMinecraft().addScheduledTask(() -> {
            ...
            player.sendMessage(
                    new TextComponentString("\u00a7c\u00a7lPlease install the missing mods to avoid issues!"));
        });
    }

    /**
     * Fires on the SERVER when any player logs in. Warns the joining player if the
     * server itself is missing any required mods.
     */
    @SubscribeEvent
    public static void onServerPlayerJoin(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        player.sendMessage(
                new TextComponentString("\u00a7e\u00a7lSome features may not work correctly on this server."));
    }

    /**
     * Returns a list of friendly mod names from REQUIRED_MODS that are not
     * currently loaded. Works on both client and server since Loader is available
     * on both sides.
     */
