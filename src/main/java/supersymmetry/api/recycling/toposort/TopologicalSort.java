...
/// registry loading, prioritization for renderers, and even mod module loading.
///
/// [CleanroomLoader](cleanroommc.com) is using a newer version of [Guava](guava.dev) library, where the [ValueGraph] no
/// longer inherits from [Graph].
@SuppressWarnings("UnstableApiUsage")
public final class TopologicalSort {

    /// A breath-first-search based topological sort.
    ///
    /// Compared to the depth-first-search version, it does not reverse the graph and supports custom secondary ordering
    /// specified by a comparator. It also utilizes the recently introduced Guava Graph API, which is more
    /// straightforward than the old directed graph.
    ///
    ...
    ///
    /// When `null` is used for the comparator and multiple nodes have no prerequisites, the order depends on the
    /// iteration order of the set returned by the [#successors(Object)] call, which is random by default.
    ///
    /// Given the number of edges `E` and the number of vertexes `V`, the time complexity of a sort without a secondary
    /// comparator is `O(E + V)`. With a secondary comparator of time complexity `O(T)`, the overall time complexity
    /// would be `O(E + TV log(V))`. As a result, the comparator should be as efficient as possible.
    ///
    ...
    public static <T> List<T> topologicalSort(ValueGraph<T, ?> graph, @Nullable Comparator<? super T> comparator)
        ...
        while (!queue.isEmpty()) {
            ...
            for (final T successor : graph.successors(current)) {
                final int updated = degrees.compute(successor, (node,
                                                                degree) -> Objects.requireNonNull(degree,
                                                                        () -> "Invalid degree present for " + node) -
                                                                        1);
                if (updated == 0) {
                    queue.add(successor);
