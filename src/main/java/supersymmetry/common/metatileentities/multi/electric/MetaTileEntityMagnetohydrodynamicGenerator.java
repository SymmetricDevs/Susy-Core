...
public class MetaTileEntityMagnetohydrodynamicGenerator extends RecipeMapMultiblockController {
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('A', states(new IBlockState[]{MetaBlocks.METAL_CASING.getState(MetalCasingType.TUNGSTENSTEEL_ROBUST)})
                        .or(autoAbilities(false, true, false, false, false, false, false).setExactLimit(1)))
                .where('B', states(new IBlockState[]{MetaBlocks.FUSION_CASING.getState(BlockFusionCasing.CasingType.FUSION_CASING)}))
