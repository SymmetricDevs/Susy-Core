...
public class MetaTileEntityHighTemperatureDistillationTower extends MetaTileEntityOrderedDT {
    ...
    @NotNull protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start(RIGHT, FRONT, UP).aisle("YSY", "YYY", "YYY").aisle("XXX", "X#X", "XXX")
                ...
                .where('X',
                        states(getCasingState())
                                .or(metaTileEntities(MultiblockAbility.REGISTRY.get(MultiblockAbility.EXPORT_FLUIDS)
                                        ...
                                        .toArray(MetaTileEntity[]::new)).setMinLayerLimited(1).setMaxLayerLimited(1))
