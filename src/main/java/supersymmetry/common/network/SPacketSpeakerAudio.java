...
public class SPacketSpeakerAudio implements IPacket, IClientExecutor {
    ...
    public static void tickTracked() {
        ...
        TRACKED.entrySet().removeIf(e -> {
            ...
            if (entity == null || entity.isDead)
                return true;
            if (!snd.playing(e.getKey()))
                return true;
            snd.setPosition(e.getKey(), (float) entity.posX, (float) entity.posY, (float) entity.posZ);
            return false;
        });
    }
    ...
    public void executeClient(NetHandlerPlayClient handler) {
        ...
        try {
            snd.newStreamingSource(false, id,
                    new URI("file", null, String.format("/speaker_%s.speaker", id), null).toURL(),
                    String.format("speaker_%s.speaker", id), false, sx, sy, sz, SoundSystemConfig.ATTENUATION_LINEAR,
                    radius);
