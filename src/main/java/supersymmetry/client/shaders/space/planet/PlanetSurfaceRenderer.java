 ...
 * Used for bodies WITHOUT an atmosphere (moon, rocky planets). Bodies WITH an
 * atmosphere use the atmosphere shader which handles terminator darkening
 * internally.
 *
 * Usage in SuSySpaceRenderer: private final PlanetSurfaceRenderer moonSurface =
 * new PlanetSurfaceRenderer();
 *
 * // In render(), before atmosphere pass: moonSurface.render(capturedView,
 * capturedProj, sunDir, planetPos, planetRadius, rotationMatrix, faceTexIds);
 * // int[6] of GL texture IDs
 */
public class PlanetSurfaceRenderer {
    ...
    public void render(float[] capturedView, float[] capturedProj, float[] sunDir, float[] planetPos, // render units
                                                                                                      // xyz
