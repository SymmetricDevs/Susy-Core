...
public class VariantFullDoorBlock<T extends Enum<T> & IStringSerializable> extends VariantBlock<T> {
    ...
    public int getMetaFromState(IBlockState state) {
        return state.getValue(VARIANT).ordinal() + (state.getValue(OPEN) ? 1 : 0) * 4 + (state.getValue(POWERED) ? 1 : 0) * 2 + state.getValue(HALF).ordinal() * 8;
    }
    ...
    {
        return state.getValue(HALF) == BlockDoor.EnumDoorHalf.UPPER ? Items.AIR : super.getItemDropped(state, rand, fortune);
    }
